...
exports.test = function (client) {
    describe('service: system error', function () {
        ...
        it('should get a auth token  error', function (done) {
            var authError = sysError.NotAuthorizedError('mp error' ,"mp out error message");
            assert(authError.body.outMsg == sysMsg.SYS_AUTH_TOKEN_ERROR,"error body message is matched ")
            assert(authError.message == sysMsg.SYS_AUTH_TOKEN_ERROR,"error message is matched ")
            done();
        });
    });
}